...
- For each holiday:
  ...
  - If Monday (1): Check if Friday before is NOT already a holiday → add as 3-day weekend bridge
  - If Tuesday (2): Check if Monday before is NOT already a holiday → add as 4-day weekend bridge
  - If Thursday (4): Check if Friday after is NOT already a holiday → add as 4-day weekend bridge
  - If Friday (5): Check if Monday after is NOT already a holiday → add as 3-day weekend bridge
  - Use subDays/addDays for date calculations
  - Format dates with format(date, 'yyyy-MM-dd')
- Detect holiday clusters: If multiple holidays within 3 days of each other, suggest workdays that connect them into mega-vacations (e.g., Wed-Thu holidays → suggest Tue+Fri for 6-day break)
...
**Key rules:**
- Tuesday and Thursday holidays create 4-day weekend bridges
- Monday and Friday holidays create 3-day weekend bridges
- Wednesday holidays alone don't create bridges, but may be part of clusters
- Detect holiday clusters (holidays within 3 days) and suggest connecting workdays
- Don't suggest bridge if that day is already a holiday
- Only national public holidays (per context decision)
...
3. **Testing:**
   ...
   - Duplicate prevention explicitly tested (edge case from truth 4)
   - **Edge case**: Test verifies no bridge suggested when adjacent day is already a holiday
...
- [ ] Test case explicitly verifies no bridge when adjacent day is already a holiday (edge case)
...
<output>
After completion, create `.planning/phases/03-full-year-calendar-performance/03-01-SUMMARY.md` following the summary template.
</output>
