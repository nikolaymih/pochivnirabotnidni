...
files_modified:
  - .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md
autonomous: false

must_haves:
  truths:
    - "Calendar loads quickly on mobile without frustrating wait"
    - "Vacation marking feels instant with no perceptible lag"
    - "Scrolling through months is smooth without janky behavior"
    - "Page content appears stable without jumping around during load"
    - "All 12 months display without timeout or crash"
  artifacts:
    - path: ".planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md"
...
@.planning/phases/03-full-year-calendar-performance/03-03-PLAN.md
...
<task type="auto">
  ...
  <files>.planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md</files>
  <action>
Create `.planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md` and perform performance measurements:
...
test -f .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md

# Contains measurements
grep -q "Bundle Size" .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md
grep -q "Lighthouse Score" .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md
grep -q "UX-01" .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md
grep -q "CAL-04" .planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md
```
  </verify>
  <done>
...
2. Verify layout:
   ...
   - [ ] Months arranged in single column (vertical scroll)
...
3. Verify holiday display:
   ...
   - [ ] Hover over holiday shows tooltip with name (desktop only)
   - [ ] Mobile: No tooltips, just colors + legend (per context decision)

4. Verify bridge day suggestions:
   - [ ] Bridge days show YELLOW background (subtle, less prominent than holidays)
   - [ ] Bridge days include: Mon→Fri (3-day), Tue→Mon (4-day), Thu→Fri (4-day), Fri→Mon (3-day)
   - [ ] Bridge days connect holiday clusters (e.g., Wed-Thu holidays suggest Tue+Fri)
   - [ ] No yellow on dates that are already holidays
   - [ ] No yellow on dates user already marked as vacation (vacation takes priority)
   - [ ] Tooltip shows specific holiday + date range (desktop only, per context)
...
**Acceptance criteria:**
- All 12 months display correctly in single column
- Holidays in red, bridge days in yellow (subtle), vacation in blue
- Bridge day logic includes 3-day and 4-day weekends + holiday clusters
- Vacation takes priority: blue shown instead of yellow when overlap
- Vacation tracking works across all months
- Desktop: Tooltips on hover; Mobile: No tooltips (colors + legend only)
- Desktop: Sticky sidebar; Mobile: Summary at top
...
<task type="auto">
  ...
  <action>
**CONDITIONAL EXECUTION:** This task runs ONLY if user reports performance issues in checkpoint OR if 03-PERFORMANCE.md shows FAIL for UX-01 or CAL-04.

**Decision tree:**

**Path A: Performance PASS (no optimization needed)**
- If UX-01 PASS (load <3s) AND CAL-04 PASS (render <200ms)
- Action: Skip optimization, document PASS status in 03-PERFORMANCE.md
- Update 03-PERFORMANCE.md with: "✓ All performance targets met. No optimization required."
- Task complete

**Path B: Performance FAIL (optimization required)**
- If UX-01 FAIL (load ≥3s) OR CAL-04 FAIL (render ≥200ms)
- Action: Apply optimizations based on severity
...
**Decision criteria:**
- If load time < 3s AND render < 200ms: Path A (no optimization)
...
- Document results in 03-PERFORMANCE.md
  ...
  <verify>
```bash
# Check which path was taken by examining 03-PERFORMANCE.md
PERF_DOC=".planning/phases/03-full-year-calendar-performance/03-PERFORMANCE.md"

# Verify document exists
test -f "$PERF_DOC" || (echo "ERROR: Performance document missing" && exit 1)

# Check for PASS/FAIL status
if grep -q "UX-01.*PASS" "$PERF_DOC" && grep -q "CAL-04.*PASS" "$PERF_DOC"; then
  # Path A: Performance targets met, no optimization needed
  echo "Path A: Performance PASS - verifying no optimization"
  grep -q "No optimization required\|No optimization needed" "$PERF_DOC" || (echo "ERROR: Should document no optimization needed" && exit 1)
  echo "✓ Path A verified"
elif grep -q "FAIL" "$PERF_DOC"; then
  # Path B: Performance targets not met, optimizations applied
  echo "Path B: Performance FAIL - verifying optimizations applied"

  # Check for optimization code in component
  if ! grep -q "dynamic\|visibleMonths\|react-window" components/FullYearCalendar.tsx package.json; then
    echo "ERROR: Performance FAIL but no optimizations found in code"
    exit 1
  fi

  # Verify re-measurement documented
  grep -q "Optimization.*applied\|After optimization" "$PERF_DOC" || (echo "WARN: Should document optimization results" && exit 0)
  echo "✓ Path B verified"
else
  echo "ERROR: Cannot determine PASS/FAIL status in $PERF_DOC"
  exit 1
fi
```
  </verify>
  <done>
**If performance targets MET (Path A):**
- No changes to components (documented in 03-PERFORMANCE.md)
- 03-PERFORMANCE.md contains PASS markers for UX-01 and CAL-04
- Performance validation confirms targets achieved without optimization

**If performance targets NOT MET (Path B):**
...
- New metrics show improvement toward targets
  </done>
</task>
...
<output>
After completion, create `.planning/phases/03-full-year-calendar-performance/03-04-SUMMARY.md` following the summary template.
</output>
