...
must_haves:
  truths:
    ...
    - "Protocol notes which tools are available vs require manual installation"
  artifacts:
...
<task type="auto">
  ...
  <action>
**Before writing the protocol, verify tool availability:**

1. Run `ab --help` (or `which ab`) to check if agent-browser is installed
2. Run `chunkhound --help` (or `which chunkhound`) to check if ChunkHound is installed
3. Record the results: which tools are available and which are not

Also read the Plan 03 SUMMARY (`05.1-03-SUMMARY.md`) to check if it contains a "## Manual Installation Required" section with installation failure details.

**Then create `.planning/ux-critique-protocol.md`** with the complete UX critique protocol. Adapt the tool references based on what is actually available:
...
{DYNAMIC: Fill this section based on the verification above}

| Tool | Status | Notes |
|------|--------|-------|
| agent-browser (`ab`) | {Installed / NOT installed} | {version or "See manual installation steps below"} |
| ChunkHound (`chunkhound`) | {Installed / NOT installed} | {version or "See manual installation steps below"} |

{If either tool is NOT installed, add:}

### Manual Installation Required

{Copy relevant details from Plan 03 SUMMARY, or note:}
- **agent-browser:** Install with `npm install -g agent-browser`. If that fails, try `cargo install agent-browser` or see https://github.com/anthropics/agent-browser
- **ChunkHound:** Install with `npm install -g chunkhound` or `pip install chunkhound`. See https://github.com/anthropics/chunkhound

**Protocol steps that reference unavailable tools should be skipped until the tool is installed.**
...
```
{If agent-browser is NOT available, note:}
> **Fallback:** If `ab` is not installed, manually open http://localhost:3000 in a browser and take screenshots for reference.
...
```
{If ChunkHound is NOT available, note:}
> **Fallback:** If `chunkhound` is not installed, use `grep -rn` to find component imports and consumers manually.
...
> **Fallback:** Manually capture screenshots after changes.
...
This document is the AUTHORITATIVE reference for the UX critique process. It will be referenced by all Phase 6 planning and execution. The Tool Availability section must reflect actual installation state, not assumptions.
  </action>
  <verify>
- File exists at .planning/ux-critique-protocol.md
- Contains all required sections: Purpose, Tool Availability, When to Trigger, Process (5 steps), Template, Rules (Allowed/Forbidden), Coffee Design System Reference
- Tool Availability section reflects actual `ab --help` and `chunkhound --help` results (not placeholders)
- If either tool is unavailable, fallback instructions are included in the relevant Process steps
...
- Token reference table includes all 9 base colors, 3 accents, 4 semantic colors
  ...
  <done>
UX critique protocol document created with complete process definition, critique template, strict "no solutions" rules, full Coffee design system reference table, AND a Tool Availability section reflecting actual installation state. If any tools are unavailable, fallback instructions are provided and manual installation steps are documented.
  </done>
</task>
...
2. .planning/ux-critique-protocol.md has Tool Availability section with actual `ab --help` / `chunkhound --help` results
3. If tools are unavailable, protocol includes fallback instructions and manual installation steps
4. .planning/coffee-token-mapping.md exists with all component color mappings
5. .planning/critiques/ directory exists (with .gitkeep)
6. Protocol explicitly states no solutions in critique output
7. Token mapping covers all components modified in Plan 02
</verification>

<success_criteria>
- UX critique protocol fully documented and enforceable for Phase 6
- Tool Availability section reflects actual installation state (not assumptions)
- Protocol provides fallback instructions when tools are unavailable
