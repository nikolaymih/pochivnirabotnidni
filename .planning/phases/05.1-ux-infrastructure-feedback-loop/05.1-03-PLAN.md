...
1. **Install agent-browser globally:**
   ...
   ```
   If installation fails (e.g., Rust binary not available for platform), document the error and fall back to noting manual installation steps. Do NOT block the rest of the plan on this.
...
2. **Install ChunkHound:**
   ...
   ```
   If neither works, document what was tried and move to MCP configuration with a note about installation pending.
...
IMPORTANT: If either tool fails to install, do NOT treat it as a plan failure. Document what happened, configure what's possible, and flag it for the checkpoint task below. The tools may need manual installation on this specific platform.
  </action>
  <verify>
Check each tool:
...
- If ChunkHound index was run, check that index artifacts exist
  ...
  <done>
agent-browser and ChunkHound installed (or installation issues documented). ChunkHound MCP server configured in .claude/settings.json alongside existing hooks/statusLine config. Initial codebase index created (if ChunkHound available).
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="non-blocking">
  ...
  <how-to-verify>
    ...
    5. If either tool failed to install, review the SUMMARY for what went wrong and any manual steps needed
  </how-to-verify>
...
<verification>
1. agent-browser installed and responds to `ab --help` (or installation failure documented)
2. ChunkHound installed and responds to `chunkhound --help` (or installation failure documented)
...
<success_criteria>
- Both tools installed or installation issues clearly documented with manual steps
