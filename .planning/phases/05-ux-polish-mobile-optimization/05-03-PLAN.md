...
Purpose: Users should be able to view the calendar with cached holidays when offline (view-only mode per CONTEXT.md). The service worker caches holiday API responses and app shell assets. PWA manifest enables mobile users to install the app to their home screen for a native-like experience.
...
<task type="auto">
  <name>Task 1: Install Serwist, create service worker, and update Next.js config</name>
  <files>app/sw.ts, next.config.ts</files>
  <action>
  First, install Serwist packages:
  ```bash
  ...
  - next.config.ts has withSerwist wrapper
  - app/sw.ts has CacheFirst for holidays, StaleWhileRevalidate for static assets
  - No Supabase URLs in caching rules
  </verify>
  <done>Service worker caches holiday API responses (CacheFirst) and static assets (StaleWhileRevalidate). Disabled in development. Build generates public/sw.js.</done>
</task>

<task type="auto">
  <name>Task 2: Create PWA manifest and update layout metadata</name>
  ...
  - Add manifest to the existing `metadata` export:
    ...
    ```
  - Add `<meta name="theme-color" content="#2563eb" />` via metadata.other or themeColor field:
    ```typescript
    export const metadata: Metadata = {
      // ... existing fields
      manifest: "/manifest.json",
