...
5. **Vacation context integration:**
   - FullYearCalendar is Server Component, doesn't access context directly
   - MonthGrid receives vacation data via props (handled by VacationContext wrapper at app level)
...
4. **Make year dynamic (per Phase 1 decision):**
   ```typescript
   const currentYear = new Date().getFullYear();
   <FullYearCalendar year={currentYear} />
   ```
...
# No longer imports old Calendar
! grep -q "import Calendar from" app/page.tsx || echo "WARNING: Old Calendar still imported"
